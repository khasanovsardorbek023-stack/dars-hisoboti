// "Gapirish savollariga berilgan javoblar" hisobot bo‘limi.
...
norm = function(s){
  ...
  if (s?.archives && typeof s.archives === 'object') {
    Object.entries(s.archives).forEach(([gid, groupArchives]) => {
      Object.entries(groupArchives || {}).forEach(([date, a]) => {
        if (!a?.reportSections || !Object.prototype.hasOwnProperty.call(a.reportSections,'speaking')) missingInOldArchives.add(gid + '|' + date);
      });
    });
  }
  ...
  (out.groups || []).forEach(g => {
    ...
    (g.students || []).forEach(st => { if (!Object.prototype.hasOwnProperty.call(st,'speaking')) st.speaking = null; });
  });
  Object.entries(out.archives || {}).forEach(([gid, groupArchives]) => {
    Object.entries(groupArchives || {}).forEach(([date, a]) => {
      ...
      (a.students || []).forEach(st => { if (!Object.prototype.hasOwnProperty.call(st,'speaking')) st.speaking = null; });
    });
  });
  return out;
};
...
freshStudents = function(g){ return baseFreshStudentsSpeaking(g).map(st => ({...st, speaking:null})); };
...
(function addSpeakingSectionToggle(){
  ...
  label.innerHTML = '<input id="sectionSpeaking" type="checkbox"> Gapirish javoblari';
...
rowHtml = function(s){
  ...
  const cell = `<div class="mobilelabel" data-label="Gapirish savollariga berilgan javoblar"><div class="status">${opt('alo','A’lo','good')}${opt('yaxshi','Yaxshi','good')}${opt('ortacha','O‘rtacha','warn')}${opt('qoniqarli','Qoniqarli','warn')}${opt('qoniqarsiz','Qoniqarsiz','bad')}</div></div>`;
...
applyReportSectionUI = function(){
  ...
  document.querySelectorAll('.srow').forEach(row => {
    ...
    const settings = [
      {index:2,key:'attendance'},{index:3,key:'vocab'},{index:4,key:'grammar'},{index:5,key:'homework'},
      {index:6,key:'speaking'},{index:7,key:'participation'},{index:8,key:'quiz'}
    ];
    settings.forEach(({index,key}) => {
      ...
      if (key === 'quiz') {
        ...
        cell.querySelectorAll('[data-act]').forEach(btn => { btn.disabled = !enabled || (key !== 'attendance' && effectivelyAbsent); });
      }
    });
  });
};
...
mainText = function(src, includePhone=false){
  ...
  const arr = (src.students || []).filter(s => String(s.name||'').trim()).map((s,i) => {
    ...
    if (sec.attendance && s.attendance === 'kelmadi') { l.push('🚫 Darsga kelmadi'); return l.join('\n'); }
    ...
    if (sec.speaking) l.push(`🗣 Gapirish savollariga berilgan javoblar: ${speakingLabel(s.speaking)}`);
